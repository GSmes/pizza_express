...
describe('Server', () => {
  ...
  describe('GET /pizzas/:id', () => {
    ...
    it('should return a page that has the toppings of a pizza listed', (done) => {
      var pizza = app.locals.pizzas.testPizza;
      ...
      this.request.get('/pizzas/testPizza', (error, response) => {
        ...
        assert(response.body.includes(pizza.toppings[2]),
           `"${response.body}" does not include "${pizza.toppings[2]}".`);
        assert(response.body.includes(pizza.toppings[3]),
           `"${response.body}" does not include "${pizza.toppings[3]}".`);
        done();
      });
    });
  });
